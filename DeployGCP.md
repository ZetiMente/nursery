...
- **Terraform only touches resources it creates.** The state file (`terraform.tfstate`) tracks what it owns; your other GCP resources are invisible to it.
...
Downloads the `google` + `http` providers. One-time.
...
5. **Multi-instance / multi-zone** — scale out.
